...
void camera_config_init(camera_config_t *config) {
  ...
  config->uBaseAddr_IIC_FmcIpmi =
      XPAR_FMC_IPMI_ID_EEPROM_0_BASEADDR; // Device for reading HDMI board IPMI
                                          // EEPROM information
  config->uBaseAddr_IIC_FmcImageon =
      XPAR_FMC_IMAGEON_IIC_0_BASEADDR; // Device for configuring the HDMI board

  // Uncomment when using VITA Camera for Video input
  config->uBaseAddr_VITA_SPI =
      XPAR_ONSEMI_VITA_SPI_0_S00_AXI_BASEADDR; // Device for configuring the
                                               // Camera sensor
  config->uBaseAddr_VITA_CAM =
      XPAR_ONSEMI_VITA_CAM_0_S00_AXI_BASEADDR; // Device for receiving Camera
                                               // sensor data
...
void camera_loop(camera_config_t *config) {
  ...
  // Grab the DMA parkptr, and update it to ensure that when parked, the S2MM
  // side is on frame 1, and the MM2S side on frame 2
  parkptr =
  ...
  volatile Xuint16 *pMM2S_Mem = (Xuint16 *)XAxiVdma_ReadReg(
      config->vdma_hdmi.BaseAddr,
      XAXIVDMA_MM2S_ADDR_OFFSET + XAXIVDMA_START_ADDR_OFFSET + 8);
