...
int vfb_rx_init(XAxiVdma *pAxiVdma, XAxiVdma_DmaSetup *pWriteCfg,
                Xuint32 uVideoResolution, Xuint32 uStorageResolution,
                Xuint32 uMemAddr, Xuint32 uNumFrames) {
  int Status;
...
int vfb_dump_registers(XAxiVdma *pAxiVdma) {
  ...
  xil_printf(
      "\t S2MM_DMACR       = 0x%08X\n\r",
      *((volatile int *)(uBaseAddr + XAXIVDMA_RX_OFFSET + XAXIVDMA_CR_OFFSET)));
  xil_printf(
      "\t S2MM_DMASR       = 0x%08X\n\r",
...
int vfb_check_errors(XAxiVdma *pAxiVdma, u8 bClearErrors) {
  ...
  // XAXIVDMA_SR_ERR_ALL_MASK; outErrors = XAxiVdma_GetStatus(pAxiVdma,
  // XAXIVDMA_READ) & XAXIVDMA_SR_ERR_ALL_MASK;
  inErrors =
      *((volatile int *)(uBaseAddr + XAXIVDMA_RX_OFFSET + XAXIVDMA_SR_OFFSET)) &
      0x0000CFF0;
  outErrors =
