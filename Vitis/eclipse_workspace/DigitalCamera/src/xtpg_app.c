...
void xTPG_main(int width, int height, int TPG_BASEADDR) {
  ...
  Xuint32 NewXHairHCoord, NewXHairVCoord, CompMaskValue, CrCbPolarity,
      NewBoxComponentVal, NewBoxSize, CurrentPattern, CurrentBoxColourPreset,
      CurrentSpeed, CurrentSPVal, CurrentNoiseGain, CurrentThrottleIndex;
  ...
  while (1) {
    ...
    case 'r': {
      ...
      Reg32Value = XTPG_mReadSlaveReg0(TPG_BASEADDR, 0);
      xil_printf(
          "   Read 0x%08x from register 0  (offset 0x0000) (PatternSel)\n\r",
          Reg32Value);
      Reg32Value = XTPG_mReadSlaveReg1(TPG_BASEADDR, 0);
      ...
      xil_printf(
          "   Read 0x%08x from register 3  (offset 0x000C) (FrameSize)\n\r",
          Reg32Value);
      Reg32Value = XTPG_mReadSlaveReg4(TPG_BASEADDR, 0);
      xil_printf("   Read 0x%08x from register 4  (offset 0x0010) (HSweep "
                 "register)\n\r",
                 Reg32Value);
      Reg32Value = XTPG_mReadSlaveReg5(TPG_BASEADDR, 0);
      xil_printf("   Read 0x%08x from register 5  (offset 0x0014) (VSweep "
                 "register)\n\r",
                 Reg32Value);
      Reg32Value = XTPG_mReadSlaveReg6(TPG_BASEADDR, 0);
      xil_printf(
          "   Read 0x%08x from register 6  (offset 0x0018) (Box Size)\n\r",
          Reg32Value);
      Reg32Value = XTPG_mReadSlaveReg7(TPG_BASEADDR, 0);
      xil_printf(
          "   Read 0x%08x from register 7  (offset 0x001C) (Box Colour)\n\r",
          Reg32Value);
      Reg32Value = XTPG_mReadSlaveReg8(TPG_BASEADDR, 0);
      xil_printf("   Read 0x%08x from register 8  (offset 0x0020) (Stuck-Pixel "
                 "frequency index)\n\r",
                 Reg32Value);
      Reg32Value = XTPG_mReadSlaveReg9(TPG_BASEADDR, 0);
      xil_printf(
          "   Read 0x%08x from register 9  (offset 0x0024) (Noise Gain)\n\r",
          Reg32Value);
      Reg32Value = XTPG_mReadSlaveReg10(TPG_BASEADDR, 0);
      xil_printf("   Read 0x%08x from register 10 (offset 0x0028) (Throttle "
                 "level)\n\r",
                 Reg32Value);
      break;
    }
    ...
    case '4': // Rotate full frame colours R G B Black White
    {
      ...
      XTPG_mWriteSlaveReg0(TPG_BASEADDR, 0,
                           (Reg32Value & 0xFFFFFFD0) | CurrentPattern);
      break;
    }

    case '5': // Rotate preset box colours White Yellow Cyan Green Magenta Red
              // Blue Black
    {
    ...
    case 'p': // Horizontal Crosshair - decrease V coordinate
    {
      ...
      XTPG_mWriteSlaveReg2(TPG_BASEADDR, 0,
                           (Reg32Value & 0xFFFF0000) | NewXHairVCoord);
      xil_printf("New XHair: (%d, %d)\n\r", (Reg32Value >> 16 & 0x0000FFFF),
                 NewXHairVCoord);
      break;
    }
    ...
    case 'a': // Vertical Crosshair - decrease H coordinate
    {
      ...
      XTPG_mWriteSlaveReg2(TPG_BASEADDR, 0,
                           (Reg32Value & 0x0000FFFF) | NewXHairHCoord << 16);
      xil_printf("New XHair: (%d, %d)\n\r", NewXHairHCoord,
    ...
    case '.': // Increment component mask
    {
      ...
      XTPG_mWriteSlaveReg0(TPG_BASEADDR, 0,
                           (CompMaskValue << 6) | (Reg32Value & 0xFFFFFE3F));
      break;
    }
    ...
    case '$': // Box Colour - decrease R/Y value
    {
      ...
      XTPG_mWriteSlaveReg7(TPG_BASEADDR, 0,
                           (Reg32Value & 0xFFFFFF00) | NewBoxComponentVal);
      break;
    }
...
void xTPG_config(int width, int height, int TPG_BASEADDR, int reset) {
  ...
  XTPG_mWriteSlaveReg3(TPG_BASEADDR, 0,
  ...
  if (reset == 0) {
    ...
    XTPG_mWriteSlaveReg2(TPG_BASEADDR, 0,
                         (573 << 16) | 360); // XHairs set at (573H, 360V)
    XTPG_mWriteSlaveReg4(
        TPG_BASEADDR, 0,
        1); // Set zone plate HDelta2 to 1 (bits 11:0)
            // Set zone plate H sin-table address offset to 0 (bits 27:16)
    XTPG_mWriteSlaveReg5(
        TPG_BASEADDR, 0,
        1); // Set zone plate VDelta2 to 1 (bits 11:0)
