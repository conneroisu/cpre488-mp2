...
int fmc_imageon_enable(camera_config_t *config) {
  ...
  ret =
      fmc_iic_axi_init(&(config->fmc_imageon_iic), "FMC-IMAGEON I2C Controller",
  ...
  if (config->bVerbose) {
    ...
    xil_printf(
        ...
        "hbp=%03d\n\r",
        config->hdmio_timing.HActiveVideo, config->hdmio_timing.HFrontPorch,
        config->hdmio_timing.HSyncWidth, config->hdmio_timing.HSyncPolarity,
        config->hdmio_timing.HBackPorch);
    xil_printf(
        "\tVSYNC Timing     = vav=%04d, vfp=%02d, vsw=%02d(vsp=%d), "
        "vbp=%03d\n\r",
        config->hdmio_timing.VActiveVideo, config->hdmio_timing.VFrontPorch,
  ...
  //vita_enable_attempt++); 	   vita_enabled_error =
...
int fmc_imageon_enable_tpg(camera_config_t *config) {
  ...
  volatile uint32_t *TPG_BOX_SIZE =
      (volatile uint32_t *)(config->uBaseAddr_TPG_PatternGenerator + 0x78);
  volatile uint32_t *TPG_BOX_COLOR_Y =
      (volatile uint32_t *)(config->uBaseAddr_TPG_PatternGenerator + 0x80);
  volatile uint32_t *TPG_BOX_COLOR_U =
      (volatile uint32_t *)(config->uBaseAddr_TPG_PatternGenerator + 0x88);
  volatile uint32_t *TPG_BOX_COLOR_V =
      (volatile uint32_t *)(config->uBaseAddr_TPG_PatternGenerator + 0x90);
  ...
  // (UNDER CONSTRUCTION: DO NOT USE!!) Useful portable wrapper functions for
  // accessing TPG configuration registers
  /*
  // See TPG data sheet for configuring the TPG for other features
  ...
  XTPG_mWriteSlaveReg0(config->uBaseAddr_TPG_PatternGenerator, 0, (Reg32Value &
  0xFFFFFFF0) | 10); ZPlate_config(config->hdmio_width, config->hdmio_height,
...
int fmc_imageon_enable_vita(camera_config_t *config) {
  ...
  vita_rate =
      config->vita_status_t2.cntFrames - config->vita_status_t1.cntFrames;
...
// XVprocSs proc_ss_RGB_YCrCb_444;  // To hold info for the Video Processing
// Subsystem: Color Conversion Only IP core: See xv_procss.h, and xv_csc_l2.h
// XVprocSs_Config *Config_ptr;

int fmc_imageon_enable_ipipe(camera_config_t *config) {

  xil_printf(
      "Hardware Image Processing Pipeline (iPIPE) Initialization ...\n\r");

  // Video Processing Subsystem (Only re-sampling) 4:4:4 to 4:2:2  (See IP
  // documentation for register details)
  // TODO Add additional register assignments here to fully configure this core.
  // See Video Processing Subsystem IP documentation for register details. Hint
  // 1: You will need to configure 4 additional registers, Hint 2: You will need
  // to dig through some header files for some of the values, Hint 3: Set up the
  // Demosaic IP block first. It is a good warm up.
  ...
  // Conversion) from 24-bit RGB to YCrCb 4:4:4 For this IP core there was a
  // High level API for setting it up.  Trace through these calls to see how
  // much work they do. This could have been set up with direct register writes,
  // but there are about 20 registers that need to be set for this IP block
  ...
  //   XV_CscSetColorspace(proc_ss_RGB_YCrCb_444.CscPtr, XVIDC_CSF_RGB,
  //   XVIDC_CSF_YCRCB_444, XVIDC_BT_709, XVIDC_BT_709, XVIDC_CR_0_255);
  ...
  // TODO Add additional register assignments here to fully configure this core.
  // See Demosaic IP documentation for register details. Hint 1: You will need
  // to configure 3 addition registers

  // Add assignment here

  // Uncomment as part of Demosaic IP setup
  //   Xil_Out32((XPAR_XV_DEMOSAIC_0_S_AXI_CTRL_BASEADDR) +
  //   (XV_DEMOSAIC_CTRL_ADDR_AP_CTRL), (u32)(0x81));// 0b10000001 means start
  //   and freerun mode (page 16 in PG286)
  xil_printf(
